...
import { Plus, UserCog, Trash2, Mail, Shield, LayoutGrid, List, Search, Phone } from "lucide-react";
          ...
          <div className="divide-y divide-border">
            {filtered.map((s: any) => (
              <Link 
                key={s.id} 
                to={`/staff/edit/${s.id}`}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 hover:bg-muted/50 transition-all group gap-4 cursor-pointer"
              >
                ...
                <div className="flex items-center justify-between sm:justify-end gap-6">
                  <Badge variant="secondary" className="bg-primary/5 text-primary border-primary/10 px-2.5 py-0.5 rounded-full text-[11px] font-bold">
                    <Shield className="w-3 h-3 mr-1.5" /> {s.role_record?.role_name || s.role || 'Staff'}
                  </Badge>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filtered.map((s: any) => (
                <Link 
                  key={s.id} 
                  to={`/staff/edit/${s.id}`}
                  className="bg-card rounded-xl border border-border p-5 hover:bg-muted/40 transition-all group relative card-shadow border-t-4 border-t-primary/20 cursor-pointer block"
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <Badge variant="outline" className="text-[10px] font-bold border-primary/20 bg-primary/5 text-primary">
                        {s.role_record?.role_name || s.role || 'Staff'}
                      </Badge>
                    </div>
                  </div>
